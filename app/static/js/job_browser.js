(() => {
  ...
  const apiUrl = (form && form.dataset.apiUrl) || "/api/v1/jobs";
  ...
  const formatSalary = (value, currency = "USD") => {
    ...
    try {
      const currencyMap = { "USD": "USD", "CHF": "CHF", "EUR": "EUR", "GBP": "GBP" };
  ...
  const updateSummary = (items, total, summaryData = null) => {
    ...
    if (summaryData && summaryData.salary && summaryData.salary.median !== null && summaryData.salary.median !== undefined) {
      ...
      salaryEl.textContent = formatted ? `Median: ${formatted}` : "Median: -";
    } else {
      salaryEl.textContent = "Median: -";
    }
  };
        ...
        cardsContainer.innerHTML = `
          <div class="text-center py-12 text-slate-500">
            ...
            <p class="text-sm mt-2">
              Try a broader title (e.g. "engineer") or remove the region filter to see more roles.
            </p>
            <button type="button"
